...
describe('Server', () => {
  ...
  describe('DELETE /api/v1/projects/:id', () => {
    it('should delete the project if it exist in the database', async () => {
      ...
      const deletedProject = await database('projects').where({id}).first();
  ...
  describe('POST /api/v1/projects', () => {
    it('should post a new project', async () => {
      ...
      const result = await database('projects')
        .where('id', response.body.id).first();
    ...
    it('should give a format error if params are incorrect', async () => {
      ...
      const missingParamsMsg = `Expected Format {project_title: <String>} 
        You are missing project_title`;

      const response = await request(app)
        .post('/api/v1/projects').send(badProject);
